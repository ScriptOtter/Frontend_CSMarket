...
import { RARE_FILTER } from "@/app/config/pages.config";
interface IProps {
  value: string[];
    ...
    >
      ...
      >
        <p className="hover:text-white mb-5">
          Раритетность {!!value.length && `(${value.length})`}
        </p>
      ...
      >
        {RARE_FILTER.map((item) => (
          ...
          >
            <Checkbox
              id={item.filter_name}
              checked={value.includes(item.filter_name)}
            ...
            />
            <div className={cn("triangle-square", item.className)}></div>
